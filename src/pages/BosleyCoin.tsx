...
const BosleyCoin = () => {
  ...
  return (
    <div className="container py-4 mx-auto max-w-7xl animate-fade-in px-2 sm:px-4">
      ...
      <div className="mb-6">
        <div 
          ...
          className="
            group flex items-center justify-between gap-2 px-3 sm:px-5 py-2 sm:py-3 rounded-xl 
        ...
        >
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            <p className="text-gray-800 font-mono text-xs sm:text-sm md:text-base truncate">
              {isMobile ? `${coinAddress.slice(0, 10)}...${coinAddress.slice(-4)}` : coinAddress}
            </p>
          </div>
          <div className="flex items-center gap-1 text-accent group-hover:text-gray-800 whitespace-nowrap">
      ...
      <Card className="p-3 sm:p-4 md:p-6">
        ...
        {isLoading ? (
          <div className="h-[250px] sm:h-[300px] md:h-[400px] w-full flex items-center justify-center bg-muted/20 rounded-lg">
          ...
          <div className="h-[250px] sm:h-[300px] md:h-[400px] w-full">
            ...
            >
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData} margin={isMobile ? { left: 0, right: 10 } : { left: 10, right: 10 }}>
                  <XAxis 
                    dataKey="date"
                  ...
                  <YAxis 
                    domain={['auto', 'auto']} 
                    tickFormatter={(value) => `$${value < 0.0001 ? value.toExponential(1) : value.toFixed(6)}`}
                    tick={{ fontSize: isMobile ? 10 : 12 }}
                    width={isMobile ? 70 : 80}
                  />
                  <Tooltip content={<CustomTooltip />} />
                  <Legend 
                    wrapperStyle={{ fontSize: isMobile ? '10px' : '12px', marginTop: '10px' }}
                  />
                  <Line 
                    type="monotone" 
                    dataKey="price" 
                    stroke="#9b87f5" 
                    strokeWidth={2} 
                    dot={{ fill: "#9b87f5", r: isMobile ? 2 : 3 }} 
                  ...
                  <Line 
                    ...
                    stroke="#7E69AB" 
                    strokeWidth={1.5} 
                    dot={{ fill: "#7E69AB", r: isMobile ? 1 : 2 }} 
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-4 mt-4 sm:mt-6">
        {isLoading ? (
          ...
          <>
            <Card className="p-2 sm:p-3 md:p-4 bg-background/50">
              <p className="text-xs sm:text-sm text-muted-foreground">Market Cap</p>
              <p className="text-sm sm:text-base md:text-lg font-bold mt-1">{coinData ? formatCurrency(coinData.marketCap) : "--"}</p>
            </Card>
            <Card className="p-2 sm:p-3 md:p-4 bg-background/50">
              <p className="text-xs sm:text-sm text-muted-foreground">24h Volume</p>
              <p className="text-sm sm:text-base md:text-lg font-bold mt-1">{coinData ? formatCurrency(coinData.volume24h) : "--"}</p>
            </Card>
            <Card className="p-2 sm:p-3 md:p-4 bg-background/50">
