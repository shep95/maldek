      ...
      <TabsList className="w-full justify-start rounded-none border-b bg-transparent">
        {["posts", "replies", "highlights", "articles", "media", "likes"].map((tab) => (
          <TabsTrigger
            ...
            value={tab}
            className={cn(
              "rounded-none border-b-2 border-transparent data-[state=active]:border-accent data-[state=active]:bg-transparent hover:text-accent transition-colors duration-300 capitalize"
            )}
          >
            {tab}
          </TabsTrigger>
